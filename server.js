...
app.post('/api/validate-photo', rateLimit(20, 60000), upload.single('photo'), function(req, res) {
  ...
  axios.post('https://api.anthropic.com/v1/messages', {
    ...
    messages: [{ role: 'user', content: [
      ...
      { type: 'text', text: 'You are a strict passport photo compliance checker.\n\nAnalyse this photo carefully. Reply ONLY with valid JSON, no markdown, no explanation.\n\nFormat: {"approved":true,"issues":[],"message":"one sentence"}\n\nCheck STRICTLY for these issues (use exact keys):\n- glasses_detected: ANY glasses, spectacles, or sunglasses present on the face — even thin metal frames, reading glasses, or partially removed glasses. Look carefully at the nose bridge and ear area for frame arms.\n- no_face: no human face visible\n- multiple_faces: more than one person\n- eyes_closed: eyes not fully open\n- head_tilted: head not straight / not facing camera directly\n- hat_or_headwear: any hat, cap, hood (religious headwear is allowed)\n- poor_lighting: face not evenly lit, harsh shadows on face\n- face_too_small: face occupies less than 50% of frame height\n- blurry: image is out of focus or motion blurred\n- mouth_open: lips not closed\n\nSet approved to false if ANY issues are found. Be strict about glasses — thin frames still fail.' }
    ]}]
